...
\end{multicols}
%---------------------- Ejercicio 3 ----------------------
\section{Ejercicio 3: B-Tree de Orden 3}

\subsection{Descripción del Problema}
Construir manualmente un B-Tree de orden 3 insertando en este orden:
...
Entregar:
\begin{itemize}
    \item Pasos detallados de cada división.
    \item Árbol final.
    \item Breve explicación de la eficiencia de los B-Tree en sistemas de archivos.
\end{itemize}

\subsection{Implementación en C++}
\begin{multicols}{2}
\begin{lstlisting}[caption={Implementación de B-Tree de Orden 3}]
#include <iostream>
#include <vector>
using namespace std;
...
private:
        ...
        void splitChild(BTreeNode* parent,
                ...
                cout << "\n  [DIVISION] Nodo lleno: ";
                for (int k : fullChild->keys)
                        cout << k << " ";
                cout << "\n  Subiendo clave "
                         << midKey << " al padre\n";

                for (size_t i = midIndex + 1;
                         i < fullChild->keys.size();
                         i++) {
                        newChild->keys.push_back(
                            fullChild->keys[i]);
                }
...
--- Paso 3: Insertar 5 ---
    ...
    [DIVISION] Nodo lleno: 10 20 
    Subiendo clave 20 al padre
...
--- Paso 8: Insertar 17 ---
    ...
    [DIVISION] Nodo lleno: 6 10 
    Subiendo clave 10 al padre
...
\end{multicols}
%---------------------- Ejercicio 4 ----------------------
\section{Ejercicio 4 (Medio): Cola Lock-Free Tipo Michael–Scott}

\subsection{Descripción del Problema}
Simular el funcionamiento de una cola lock-free (sin bloqueos) para dos hilos siguiendo el algoritmo de Michael–Scott.  
Los hilos realizan las siguientes operaciones:

\begin{itemize}
    \item Hilo 1 encola: 1, 2
    \item Hilo 2 encola: 3, 4
\end{itemize}

El estudiante debe entregar:
\begin{enumerate}
    \item Estado del puntero \texttt{tail} tras cada operación \texttt{CAS}.
    \item Lista final almacenada en la cola.
    \item Explicación del problema ABA.
\end{enumerate}

\subsection{Implementación en C++}
\begin{multicols}{2}
\begin{lstlisting}[caption={Cola Lock-Free tipo Michael–Scott}]
#include <iostream>
#include <atomic>
#include <thread>
#include <vector>
...
public:
    ...
    void explainABA() {
        cout << "\n========== "
                "PROBLEMA ABA ==========\n";
        cout << "El problema ABA ocurre "
                "cuando:\n\n";
        cout << "1. Hilo T1 lee tail "
                "apuntando a nodo A\n";
        cout << "2. T1 es suspendido antes "
                "de hacer CAS\n";
        cout << "3. Hilo T2 desencola A, "
                "luego B, luego encola\n";
        cout << "   un nuevo nodo que "
                "reutiliza la direccion A\n";
        cout << "4. T1 se reanuda, ve que "
                "tail sigue siendo A\n";
        cout << "   (misma direccion)\n";
        cout << "5. CAS tiene exito aunque "
                "la estructura cambio\n";
        cout << "   entre medio\n\n";
        cout << "Solucion: usar contadores "
                "de version (tagged\n";
        cout << "pointers) o hazard "
                "pointers.\n";
        cout << "En esta simulacion "
                "simplificada no ocurre\n";
        cout << "porque no hay dequeue ni "
                "reuso de memoria.\n";
        cout << "======================="
                "============\n";
    }
...
int main() {
    ...
    cout << "Estado inicial: cola "
            "vacia con nodo dummy\n";
...
===============================
Simulacion con 2 hilos:
  Hilo 1: encola 1, 2
  Hilo 2: encola 3, 4

Estado inicial: cola vacia con nodo dummy

--- Hilo 2: Encolando 3 ---
  [CAS #1] next de 0 -> 3
    -> tail apunta a nodo con valor: 0
  [CAS #2] tail avanzado a 3
    -> tail apunta a nodo con valor: 3

--- Hilo 1: Encolando 1 ---
  [CAS #3] next de 3 -> 1
    -> tail apunta a nodo con valor: 3
  [CAS #4] tail avanzado a 1
    -> tail apunta a nodo con valor: 1
\end{verbatim}
\columnbreak
\begin{verbatim}
--- Hilo 2: Encolando 4 ---
  [CAS #5] next de 1 -> 4
    -> tail apunta a nodo con valor: 1
  [CAS #6] tail avanzado a 4
    -> tail apunta a nodo con valor: 4

--- Hilo 1: Encolando 2 ---
  [CAS #7] tail corregido (estaba atrasado)
    -> tail apunta a nodo con valor: 4
  [CAS #8] next de 4 -> 2
    -> tail apunta a nodo con valor: 4
  [CAS #9] tail avanzado a 2
    -> tail apunta a nodo con valor: 2

========== LISTA FINAL ==========
Contenido de la cola: 3 -> 1 -> 4 -> 2
=================================
\end{verbatim}
\end{multicols}
\end{document}
